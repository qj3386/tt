...
class UserLogic extends BaseLogic
{
    ...
    public function login($data)
    {
		...
		if (!$this->getModel()->getOne(['mobile'=>$user_name])) {
			return ReturnData::create(ReturnData::PARAMS_ERROR, null, '该账号尚未注册');
		}
        //用户名/手机号/邮箱+密码
		...
		if ($user['status'] == 1) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, '等待审核');
        }
    ...
    public function userInfoUpdate($data, $where = array())
    {
        ...
        $validate = new Validate([
			...
			['true_name', 'max:20', '姓名格式不正确'],
			['idcard', 'length:18', '身份证号格式不正确'],
			['bank_name', 'max:60', '银行名称格式不正确'],
			['bank_branch_name', 'max:60', '支行名称格式不正确'],
			['bank_card_number', 'max:30', '卡号格式不正确'],
			['status', 'in:0,1,2,3', '用户状态：0正常，1待审，2锁定'],
            ['is_auto_reinvest', 'in:0,1,2', '是否自动复投，0否1是'],
        ]);
        if (!$validate->check($data)) {
            return ReturnData::create(ReturnData::FAIL, null, $validate->getError());
        }
    ...
    public function userPayPasswordUpdate($data, $where = array())
    {
        ...
        $data['pay_password2'] = $data['pay_password'];
    ...
    public function signin($where)
    {
        ...
        if ($signin_time == $today) {
            return ReturnData::create(ReturnData::FAIL, null, '今日已签到，请明日再来');
        }

        $signin_point = sysconfig('CMS_SIGN_POINT'); //签到积分/金额
        $res = logic('UserMoney')->add(array('type' => 0, 'money' => $signin_point, 'desc' => '签到奖励', 'user_id' => $user['id'])); //添加签到积分记录，并增加用户积分
        if ($res['code'] != ReturnData::SUCCESS) {
            return ReturnData::create(ReturnData::FAIL, null, $res['msg']);
        }
        $this->getModel()->edit(array('signin_num' => ($user['signin_num'] + 1), 'signin_time' => $time), array('id' => $user['id'])); //更新签到时间
        db('sysconfig')->where(['varname' => 'CMS_SIGN_PERSON_TIMES'])->setInc('value', 1);
    ...
    /**
     * 分佣计算
     * @param int $user_id 消费用户ID
     * @param float $money 消费金额
     * @return array
     */
    public function dividend_calculation($user_id, $money, $fenxiao = [])
    {
		...
		//一级分销奖励比例
		$fenxiao_yiji = 0;
		if (isset($fenxiao['distribution_yiji']) && $fenxiao['distribution_yiji'] > 0) {
			$fenxiao_yiji = $fenxiao['distribution_yiji'];
		}
		if ($fenxiao_yiji > 0) {
			//增加推荐人余额
			$user_money_data = [];
			$user_money_data['user_id'] = $user['parent_id'];
			$user_money_data['type'] = 0; // 0增加,1减少
			$user_money_data['money'] = round(($money * $fenxiao_yiji / 100), 2);
			$user_money_data['desc'] = '一级邀请奖励';
			$res = logic('UserMoney')->add($user_money_data);
			//增加累计佣金金额
			model('User')->setIncrement(array('id' => $user_money_data['user_id']), 'commission', $user_money_data['money']);
		}
        $yiji_user = model('User')->getOne(['id' => $user['parent_id']]);
        //二级分佣
        if ($yiji_user['parent_id'] == 0) {
            return true;
        }

		//二级分销奖励比例
		$fenxiao_erji = 0;
		if (isset($fenxiao['distribution_erji']) && $fenxiao['distribution_erji'] > 0) {
			$fenxiao_erji = $fenxiao['distribution_erji'];
		}
		if ($fenxiao_erji > 0) {
			//增加二级推荐人余额
			$user_money_data = [];
			$user_money_data['user_id'] = $yiji_user['parent_id'];
			$user_money_data['type'] = 0; // 0增加,1减少
			$user_money_data['money'] = round(($money * $fenxiao_erji / 100), 2);
			$user_money_data['desc'] = '二级邀请奖励';
     ...
     * @return array
     */
    public function user_rank_calculation($user_id)
    {
        $user = model('User')->getOne(['id' => $user_id]);
        if (!$user) {
            return true;
        ...
        $user_rank = model('UserRank')->getOne(['min_points' => ['<=', $user['consumption_money']], 'max_points' => ['>', $user['consumption_money']]]);
        if (!$user_rank) {
            return true;
        }

        $res = model('User')->edit(['user_rank' => $user_rank['rank']], ['id' => $user['id']]);
        if (!$res) {
            return false;
        }
