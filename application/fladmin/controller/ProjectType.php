...
use app\common\logic\ProjectTypeLogic;
...
class ProjectType extends Base
{
    ...
    {
        return new ProjectTypeLogic();
    }

    //列表
    public function index()
    {
        $list = model('ProjectType')->tree_to_list(model('ProjectType')->list_to_tree());
    ...
    public function add()
    {
        ...
        if ($parent_id != 0) {
            ...
            $this->assign('parent_project_type', model('ProjectType')->getOne("id=$parent_id", ['content']));
        }
    ...
    public function del()
    {
        ...
        if (model('Project')->getCount(['type_id' => $id]) > 0) //判断该分类下是否有文章，如果有把该分类下的文章也一起删除
        {
            if (model('Project')->del(['type_id' => $id])) {
